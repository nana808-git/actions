...
module "pipeline" {
  ...
  cluster_name                   = "${var.app["name"]}"
  environment                    = "${var.app["env"]}"
  image                          = "module.ecs-infra.repository_url:latest"
  codestar_connector_credentials = var.codestar_connector_credentials
  container_name                 = "${var.app["name"]}-${var.app["env"]}-node-api"
  app_repository_name            = module.ecs-infra.repository_name
  repository_url                 = module.ecs-infra.repository_url
  repository_name                = module.ecs-infra.repository_name
  app_service_name               = module.ecs-infra.service_name
  environment_variables          = var.environment_variables
  vpc_id                         = module.vpc.id
  db_endpoint                    = module.ecs-infra.db_endpoint
  s3-bucket                      = module.ecs-infra.s3-bucket
